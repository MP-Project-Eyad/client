...
//   }
  ...
  return (
    ...
    <div className="restWrapper">
          ...
          <Box >
              <VStack>
          <Input textAlign="center" width="80" mt="39" bg="#444" placeholder="Search..." fontSize="1.5rem" color="white" /></VStack>
     <SimpleGrid padding="3rem" columns={3} spacing={10}>
      {restaurants.map((item, i) => (<>
        <VStack className="mainDiv">
          ...
          padding="20px"
          borderRadius="7">
          <Text fontSize="2rem" fontFamily="Hardware" key={`desc-${item._id}`} >{item.Name}</Text>
     

          <Text key={`img-${item._id}`}><Image
  borderRadius='full'
  boxSize='300px'
  src={item.Picture}
  alt='Dan Abramov'
/></Text>
 <Button
 mt="6" ml="170" bg="#222" color="white" _hover={{
    background: "white",
    color: "black",
  }}>Show Offers</Button>
